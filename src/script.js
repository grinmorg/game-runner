...
function createScene() {
  ...
  hpText.style.backgroundColor = "green";
  hpText.innerHTML = `You have: <b class="big-text">${healthPoints}</b> Life`;
  hpText.style.top = 80 + "px";
...
function decreaseHP() {
  if (!throttleHP) {
    ...
    if (healthPoints === 0) {
      ...
      endText.style.height = '100%';
      endText.style.backgroundColor = "black";
      endText.innerHTML = "<p class='end-text'>GAME OVER. If you can play again - reload page :)</p>";
      endText.style.top = 0;
      endText.style.left = 0;
      document.body.appendChild(endText);
    }

    hpText.innerHTML = `You have: <b class="big-text">${healthPoints}</b> Life`;
    console.log("Теперь кол-во хп:" + healthPoints);
